...
class UploadConsumer(AsyncWebsocketConsumer):
        ...
        if not self.form_over_stat:
            await self.channel_layer.group_send(
                self.upload_group_name,
                {
                    ...
                    "column_content": {"overall_status": "", "error": ""},
                },
            )
        else:
            if extension_format == ".csv":
                ...
                data = pd.read_csv(
                    file, skiprows=int(self.template.skiprows), delimiter="\t"
                )
            else:
                data = pd.read_excel(file, skiprows=int(self.template.skiprows))
            df = pd.DataFrame(data)
            self.source_column_names_raw = {
                k: v
                for k, v in sorted(
                    json.loads(self.template.source_column_names).items(),
            ...
            self.wrong_cols = ", ".join(
                list(
                    ...
                    - set(self.source_column_names.split(", "))
                )
            )
            if self.template.table == "pro_költségek":
                ...
                missing_cols = [i for i in missing_cols if i not in cols_to_remove]
            else:
                missing_cols = list(
                    set(self.source_column_names.split(", "))
                    - set(self.gotten_column_names.split(", "))
            ...
            numeric_cols_source = (
                [
                    j
                    for i, j in self.source_column_names_raw.items()
            ...
            date_cols_source = (
                [j for i, j in self.source_column_names_raw.items() if i in date_cols]
                if date_cols
                else []
            )
            ...
            for i in df.columns:
                try:
                    ...
                    column_content_error.append({"error_col": str(i), "error": str(e)})
            await self.channel_layer.group_send(
                self.upload_group_name,
                {
                    "type": "status",
    ...
    async def disconnect(self, code):
        await self.channel_layer.group_discard(
            self.upload_group_name, self.channel_name
        )
...
class SMOrderConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        await self.accept()
        await self.channel_layer.group_add("sm_order", self.channel_name)
    ...
    async def order_status_change(self, event):
        await self.send(
            text_data=json.dumps(
                {"message": event["message"], "progress_value": event["progress_value"]}
            )
        )

    async def disconnect(self, code):
        await self.channel_layer.group_discard("sm_order", self.channel_name)
