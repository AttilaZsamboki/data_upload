...
stock_transaction_report.sort_values(by="Finished", ascending=False, inplace=True)
...
stock_report = pd.read_sql_query(
    "select * from fol_stock_report where timestamp = (select max(timestamp) from fol_stock_report)",
    con=engine,
)
...
for row in stock_report.iloc:
    ...
    if sku in skus and sku not in stock_report_obj:
        stock_report_obj[sku] = {
            "on_stock": row["on_stock"],
            "inventory_value_layer": row["inventory_value_layer"],
            "on_stock_layer": row["on_stock_layer"],
        }
        ...
        if current_stock > 0:
            ...
            for i in infos:
                if type(i) is list:
                    shipment = i[0]
                    shipment_date = (
                        datetime.now() - datetime.strptime(str(i[1]), "%Y-%m-%d %H:%M")
                        if str(i[1]) != "nan"
                        else "nan"
                    )
                    date = (
                        datetime.strptime(str(i[1]), "%Y-%m-%d %H:%M")
                        if str(i[1]) != "nan"
                        else "nan"
                    )
                    ...
                    if shipment >= current_stock and current_stock:
                        ...
                        stock_aging["weighted_archimetric_mean"].append(
                            inventory_value_layer / on_stock_layer
                            if on_stock_layer
                            else 0
                        )
