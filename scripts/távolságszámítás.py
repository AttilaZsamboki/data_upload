...
df = pd.read_sql(
    "select * from pen_adatlapok_rendezett where \"Adatlap_Azonosito\" not in (select id from pen_kiszállások);", con=engine)
...
for i in df.iloc:
    if not current_day or i["Adatlap_Ki_epitette_be?"] not in current_day.keys():
        direction_result = gmaps.directions(
            telephely, i["Adatlap_Cim:"], mode="driving")
        distance = direction_result[0]['legs'][0]['distance']['value']
        duration = direction_result[0]['legs'][0]['duration']['value']
        obj.append({"nap": i["Adatlap_Beepites_Idopontja"], "honnan": telephely, "hova": i["Adatlap_Cim:"], "kihez": i["Adatlap_Nev"],
        ...
        direction_result = gmaps.directions(last_at[i["Adatlap_Ki_epitette_be?"]], i["Adatlap_Cim:"],
                                            mode="driving")
        distance = direction_result[0]['legs'][0]['distance']['value']
        duration = direction_result[0]['legs'][0]['duration']['value']
        obj.append({"nap": i["Adatlap_Beepites_Idopontja"], "honnan": last_at[i["Adatlap_Ki_epitette_be?"]], "hova": i["Adatlap_Cim:"], "kihez": i["Adatlap_Nev"],
                    "kik": i["Adatlap_Ki_epitette_be?"], "ut_hossz_m": distance, "ut_hossz_s": duration, "id": i["Adatlap_Azonosito"], "adatlap_url": i["Adatlap_URL"]})
    else:
    ...
    last_at[i["Adatlap_Ki_epitette_be?"]] = i["Adatlap_Cim:"]
